...
func TestExpectIdentityValueMatchesQueryAtPath_CheckQuery_ResourceNotFound(t *testing.T) {
	t.Parallel()
...
func TestExpectIdentityValueMatchesQueryAtPath_CheckQuery_No_Terraform_Identity_Support(t *testing.T) {
	t.Parallel()
...
func TestExpectIdentityValueMatchesQueryAtPath_CheckQuery_No_Identity(t *testing.T) {
	t.Parallel()
...
func TestExpectIdentityValueMatchesQueryAtPath_CheckQuery_String_Matches(t *testing.T) {
	t.Parallel()
...
func TestExpectIdentityValueMatchesQueryAtPath_CheckQuery_String_DoesntMatch(t *testing.T) {
	t.Parallel()
		...
		Steps: []r.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`expected identity \(examplecloud_thing.one.id\) and query value \(examplecloud_thing.one.id\) to match, but they differ: identity value: id-123, query value: 321-di`),
			},
		},
	})
}

func TestExpectIdentityValueMatchesQueryAtPath_CheckQuery_List(t *testing.T) {
	t.Parallel()
		...
		Steps: []r.TestStep{
			{
				...
				ConfigQueryChecks: []querycheck.QueryCheck{
					querycheck.ExpectIdentityValueMatchesQueryAtPath(
						...
						tfjsonpath.New("query_list_of_numbers"),
					),
				},
			},
		},
...
func TestExpectIdentityValueMatchesQueryAtPath_CheckQuery_List_DoesntMatch(t *testing.T) {
	t.Parallel()
		...
		Steps: []r.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`expected identity \(examplecloud_thing.one.list_of_numbers\) and query value \(examplecloud_thing.one.list_of_numbers\) to match, but they differ: identity value: \[1 2 3 4\], query value: \[4 3 2 1\]`),
			},
		},
	})
}
					...
					NewQuery: tftypes.NewValue(
						tftypes.Object{
							AttributeTypes: map[string]tftypes.Type{
								"name":                  tftypes.String,
								"query_id":              tftypes.String,
								"query_list_of_numbers": tftypes.List{ElementType: tftypes.Number},
							},
						},
						map[string]tftypes.Value{
							"name":     tftypes.NewValue(tftypes.String, "test value"),
							"query_id": tftypes.NewValue(tftypes.String, "id-123"),
							"query_list_of_numbers": tftypes.NewValue(
