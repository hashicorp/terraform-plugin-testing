...
func (s TestStep) validate(ctx context.Context, req testStepValidateRequest) error {
	...
	if req.StepConfiguration == nil && !s.ImportState && !s.RefreshState {
		...
		logging.HelperResourceError(ctx, "TestStep validation error", map[string]any{logging.KeyError: err})
		return err
	}
	...
	if err != nil {
		logging.HelperResourceError(ctx, "TestStep error checking for providers", map[string]any{logging.KeyError: err})
		return err
	}
	...
	if req.StepConfiguration != nil {
		...
		if err != nil {
			logging.HelperResourceError(ctx, "TestStep error checking for if configuration has provider block", map[string]any{logging.KeyError: err})
			return err
		}
	}
