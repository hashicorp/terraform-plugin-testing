	...
	r.Test(t, r.TestCase{
		...
		Steps: []r.TestStep{
			...
			},
			{
				...
				ExpectError: regexp.MustCompile(`expected at least two values to be the same, but all values differ`),
			},
		},
	})
}
	...
	r.Test(t, r.TestCase{
		...
		Steps: []r.TestStep{
			...
			},
			{
				...
				ExpectError: regexp.MustCompile(`expected values to differ, but value is duplicated: true`),
			},
		},
	})
}
